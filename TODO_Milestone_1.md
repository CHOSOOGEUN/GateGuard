...
## 김민지 (DB) [완료]
- [x] DB 컨테이너 접속 후 TimescaleDB 확장 활성화 + `events` 하이퍼테이블 변환 (완료: 2026-03-29)
- [x] Alembic `env.py` async 엔진 연결 (조수근과 함께 완료: 2026-03-27)
- [x] `seed.py` 작성 → 관리자 계정 + 테스트 카메라 데이터 삽입 (완료: 2026-03-29)
- [x] Swagger에서 로그인 → 이벤트 생성까지 흐름 확인 (완료: 2026-03-29)
