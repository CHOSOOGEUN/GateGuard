...
GateGuard는 기존 역무원의 육안 감시를 대체하여, **CCTV 영상을 AI가 실시간 분석**하고 무임승차(뒤따라 들어오기, 점프, 비상문 이용 등)를 자동 감지해 **관리자 대시보드에 즉시 실시간 알림(WebSocket)**을 전송하는 최첨단 보안 시스템입니다.
...
| ByteTrack | 다중 객체 ID 추적 (v0.27.0) |
| Supervision | 라인 크로싱 / 구역 통과 감지 |
| OpenCV | 영상 전처리 및 비식별화 (Gaussian Blur) |
...
| PostgreSQL + TimescaleDB | 시계열 이벤트 데이터 저장 및 압축 |
...
- `develop`: 통합 개발 브랜치
- `feature/담당자-기능명`: 개별 기능 전용 브랜치

> **[현재 상태]** Milestone 1.0 (Core & AI Integration) **전격 완수 (2026-04-01)** 🏁  
> - 백엔드 코어 인프라 & 보안 체계 구축 완료 (조수근)  
> - AI 파이프라인(ByteTrack + Face Anonymization) 통합 완료 (윤효정)  
> - TimescaleDB 시계열 최적화 및 고속 조회 시스템 구축 완료 🛡️
