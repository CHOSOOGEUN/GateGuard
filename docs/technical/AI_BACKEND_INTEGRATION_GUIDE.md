# 🛰️ GateGuard — AI-Backend API Integration Guide (v2.0)
...
## 🛠️ 4. 연동 시 주의 사항 (Best Practices)

1. **S3 선-업로드**: 영상 클립을 먼저 S3에 업로드한 뒤, 발급된 URL을 포함하여 API를 호출하는 것이 권장됩니다.
2. **비식별화 처리**: 전송되는 영상 클립은 반드시 `OpenCV` 등을 통해 얼굴 블러링 처리가 완료된 상태여야 합니다 (비식별화 모듈 연동 필수).
3. **에러 핸들링**: 네트워크 순간 단절 시 재시도(Retry) 로직을 포함하여 데이터 유실을 방지합니다.

---

## 🏁 최종 업데이트 일시: 2026-03-31 16:30 (KST, v2.0)
